    ...
    public void run() {
        ...
        try {
            ...
            for(int i=0; i < 100; i++) {
                ...
                //Hence the updates for a given country are sequential
                String recKey = String.valueOf(country);

                ProducerRecord<String, String> record = new ProducerRecord<String, String>(topic, recKey, value );

                RecordMetadata rmd = myProducer.send(record).get();

                System.out.println("Kafka Gaming Stream Generator : Sending Event : " + recKey + " = " + value );
